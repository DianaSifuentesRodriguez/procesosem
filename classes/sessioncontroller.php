<?php
    ...
    class SessionController extends Controller{
        ...
        function validateSession(){
            ...
            if($this->existsSession()){
                ...
                if($this->isPublic()){
                   ...
                   //y header('location: '. constant('URL') . '');
                }
            }
        }
        /**
        ...
        public function initialize($user){
            ...
            $this->session->setCurrentUser($user->getusu_cod());
            
            $this->session->setCurrentRUC($user->getRuc());
        ...
        private function redirectDefaultSiteByRole($role){
            ...
            for($i = 0; $i < sizeof($this->sites); $i++){
                if($this->sites[$i]['role'] === $role){
                    $url = '/procesosem/'.$this->sites[$i]['site'];
                break;
                }
            }
            header('location: '.$url);
        ...
        function authorizeAccess($role){
            ...
            switch($role){
                case '03':
                    $this->redirect($this->defaultSites['03']);
                break;
                case '02':
                    $this->redirect($this->defaultSites['02']);
                break;
                case '01':
                    $this->redirect($this->defaultSites['01']);
                break;
                default:
            }
        }
